#!/usr/bin/env node
/**
 * Qualifies the compiled Node addon for this host: loads it through
 * `packages/javascript`'s real public API — the same `#native` import a
 * consumer resolves, not a test double — and asserts one canonical-corpus
 * fixture end to end (issue #74, acceptance criterion 7).
 *
 * Preconditions:
 ...
 *
 * `packages/javascript` declares this host's addon as one of six
 * `optionalDependencies` (issue #79), resolved by
 * `runtime/node.ts`'s `resolveAddonSpecifier()`. Nothing publishes those
 * packages in this qualification run, so this script links the local build
 * in under that exact resolved name itself, through a symlink under
...
async function main() {
  ...
  const { resolveAddonSpecifier } = await import(
    pathToFileURL(join(PACKAGE_ROOT, "dist/runtime/node.js")).href
  );
  ...
  const addonLink = join(ADDON_LINK_SCOPE_DIR, specifier.split("/")[1]);

  await rm(addonLink, { recursive: true, force: true });
  await mkdir(ADDON_LINK_SCOPE_DIR, { recursive: true });
  await symlink(ADDON_TARGET, addonLink, "dir");
  try {
